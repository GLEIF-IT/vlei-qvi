...
kli vc issue --name "${QAR_NAME}" --passcode "${passcode}" --alias "${QAR_AID_ALIAS}" --registry-name "${QAR_REG_NAME}" --schema ENPXp1vQzRF6JwIuS-mp2U8Uf1MoADoP_GqQ62VsDZWY --recipient "${recipient}" --data @"data/legal-entity-data.json" --edges @"data/legal-entity-edge-data.json" --rules @"data/rules.json" --out "data/le.json"
